# Oh_Why-s-NewTab
这是是一个小插件，让浏览器变得整洁！

随缘维护，主要用于学习喵

***

# 有什么用呢？
首先再次介绍一下这个插件，是一个本地优先，仿照手机桌面风格的edge扩展，是一个`纯HTML/CSS/JS`的工具，没有后端服务也没有依赖（还没学会QwQ）

初衷是摆脱edge新建标签页的混乱，同时也希望可以`自定义背景图`，还要让新建标签页看上去并不单调。

同时，因为是仿效的手机端桌面，主要是`创建桌面文件夹`希望可以让快捷方式的管理更方便。

为了更大程度的保证自由度，大部分设置还是可以自定义的，不过这也就导致了你想要真正做出能让自己满意的桌面还是需要费点时间的。所以现在正在想办法让默认设置可以更舒服些。

无聊的3个小彩蛋应该没人看吧（-_-!）

总之，我在有空时会把它不断改进，让它成为为我所定义的真正好用的NewTab插件。

# 怎么安装？

  &emsp;&emsp;1. 在右侧的`Releases`里你可以找到你想下载的版本，然后找到该版本模块最下面的`Assets`并展开，找到**OhWhy.s.NewTab.zip**点击即可下载。
  
  &emsp;&emsp;2. 下载后把你的zip放到一个你准备好的地方，并解压。

  &emsp;&emsp;3. 接下来是`edge`部分喽，打开你的edge，点击左上角的 " ... "（或 `Alt + F`），点击`扩展`-->`管理扩展`。<img width="2130" height="1077" alt="583d748b8272bb23227226c2d9367059" src="https://github.com/user-attachments/assets/0eadb851-4fc7-4adb-bd63-760dd3b62128" />

  
  &emsp;&emsp;4. 进入`管理扩展页面`后，先打开左下角的`开发人员模式`，再点击`加再解压缩的扩展`。<img width="2541" height="1299" alt="d3559c401a99e4b181d4752ae568730d" src="https://github.com/user-attachments/assets/2cb4b54e-9e8e-4180-b56e-ff94134e1178" />

  &emsp;&emsp;5. 找到你刚刚把`zip`放到的位置<img width="1263" height="609" alt="778373c827631c7b6a7384b6d86b98ea" src="https://github.com/user-attachments/assets/8c57d164-fd99-42c1-94ff-6b5d22eb1f1c" />这个时候NewTab插件也就放进了你的edge插件里啦！

  &emsp;&emsp;6. 回到刚刚的`管理扩展页面`，你就可以看到一个名为`Edge Beautiful Newtab`的扩展，启用它，再新建标签页就可以看到扩展生效了喵。
<img width="2549" height="1191" alt="8741dcf355475e3983816b25e454e1e8" src="https://github.com/user-attachments/assets/20c053fe-0832-48b7-ab58-b9ed2ec0c974" />
你就可以看到被更改的新建标签页了喵。
<img width="2549" height="1191" alt="585b6b7ea73db37db7a246fe530e5814" src="https://github.com/user-attachments/assets/9ebb6258-6aaa-4487-ae33-940eabb1d8eb" />

  

